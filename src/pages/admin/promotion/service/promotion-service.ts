...
export const useGetPromotionCandidates = (organizationId: string, academicYearId: string, sectionId: string) =>
  useQuery<{ items: IPromotionCandidate[] }, IAPIError>(
    [API_QUERY_KEY.GET_PROMOTION_CANDIDATES, organizationId, academicYearId, sectionId],
    async () => {
    ...
    { enabled: !!organizationId && !!academicYearId && !!sectionId, cacheTime: 0 }
  );
...
export const useProcessPromotions = (organizationId: string) =>
  useMutation<{ items: IPromotionDecisionResult[] }, IAPIError, IPromotionDecisionInput[]>(async (decisions) => {
