...
>(({ className, ...props }, ref) => {
  ...
  return (
    <LabelPrimitive.Root
      ref={ref}
      htmlFor={formItemId}
      className={cn("block text-sm font-medium text-gray-700 mb-1", error && "text-red-600", className)}
      {...props}
    />
  );
});
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    if (!body) return null;
    return (
      <p ref={ref} id={formMessageId} className={cn("text-sm text-red-600 mt-1", className)} {...props}>
        {body}
      </p>
    );
  }
